...
def draw_point_cloud(points_3d, points_3d_true, features, output_filename, x_limit=None, y_limit=None, z_limit=None):
    ...
    ax.legend(handles=legend_elements, loc='upper center', fontsize=10, frameon=False, handlelength=2, handletextpad=1,
              labelspacing=0.5, ncol=6, bbox_to_anchor=(0.5, 1))
    plt.tight_layout()
...
if __name__ == "__main__":
    ff = open(
        'experiments/test/test_calcs/true.pickle',
        'rb')
    true = pickle.load(ff)
    ff.close()

    ff = open(
        'experiments/test/test_calcs/pred.pickle',
        'rb')
    pred = pickle.load(ff)
    ff.close()

    ff = open(
        'experiments/test/test_calcs/sequences.pickle',
        'rb')
    seqs = pickle.load(ff)
    ff.close()
